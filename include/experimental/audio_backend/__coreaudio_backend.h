...
// TODO: make __coreaudio_sample_type flexible according to the recommendation
// (see AudioSampleType).
...
public:
  ...
  bool set_buffer_size_frames(buffer_size_t new_buffer_size) {
    // TODO: for some reason, the call below succeeds even for nonsensical
    // buffer sizes, so we need to catch this  manually:
    if (new_buffer_size < _min_supported_buffer_size ||
        new_buffer_size > _max_supported_buffer_size)
      return false;
  ...
  template <typename _CallbackType,
            typename = enable_if_t<is_nothrow_invocable_v<
                _CallbackType, audio_device &,
  ...
  // TODO: remove std::function as soon as C++20 default-ctable lambda and
  // lambda in unevaluated contexts become available
  ...
  bool start(
    ...
    if (!_running) {
      ...
      if (!__coreaudio_util::check_error(
              AudioDeviceStart(_device_id, _device_callback))) {
        __coreaudio_util::check_error(
            AudioDeviceDestroyIOProcID(_device_id, _proc_id));
  ...
  bool stop() {
    if (_running) {
      if (!__coreaudio_util::check_error(
              AudioDeviceStop(_device_id, _device_callback)))
        return false;
...
private:
  ...
  audio_device(device_id_t device_id, string name,
               __coreaudio_stream_config config)
      : _device_id(device_id), _name(move(name)), _config(config) {
    assert(!_name.empty());
    assert(config.input_config.mNumberBuffers == 0 ||
           config.input_config.mNumberBuffers == 1);
    assert(config.output_config.mNumberBuffers == 0 ||
           config.output_config.mNumberBuffers == 1);
  ...
  static OSStatus _device_callback(AudioObjectID device_id,
    ...
    _fill_buffers(input_data, input_time, output_data, output_time,
                  this_device._current_buffers);

    invoke(this_device._user_callback, this_device,
           this_device._current_buffers);
    return noErr;
  }

  static void _fill_buffers(
      const AudioBufferList *input_bl, const AudioTimeStamp *input_time,
      const AudioBufferList *output_bl, const AudioTimeStamp *output_time,
    ...
    if (num_output_buffers == 1) {
      buffers.output_buffer =
          coreaudio_buffer_to_buffer(output_bl->mBuffers[0]);
      buffers.output_time = coreaudio_timestamp_to_timepoint(output_time);
    }
  }

  static audio_buffer<__coreaudio_native_sample_type>
  coreaudio_buffer_to_buffer(const AudioBuffer &ca_buffer) {
    // TODO: allow different sample types here! It will possibly be int16_t
    // instead of float on iOS!

    auto *data_ptr =
        reinterpret_cast<__coreaudio_native_sample_type *>(ca_buffer.mData);
    const size_t num_channels = ca_buffer.mNumberChannels;
    const size_t num_frames = ca_buffer.mDataByteSize /
                              sizeof(__coreaudio_native_sample_type) /
                              num_channels;

    return {data_ptr, num_frames, num_channels, contiguous_interleaved};
  }

  static audio_clock_t::time_point
  coreaudio_timestamp_to_timepoint(const AudioTimeStamp *timestamp) {
  ...
  void _init_supported_buffer_sizes() {
    AudioObjectPropertyAddress pa = {kAudioDevicePropertyBufferFrameSizeRange,
                                     kAudioObjectPropertyScopeGlobal,
                                     kAudioObjectPropertyElementMain};
    ...
    _min_supported_buffer_size =
        static_cast<buffer_size_t>(buffer_size_range.mMinimum);
    _max_supported_buffer_size =
        static_cast<buffer_size_t>(buffer_size_range.mMaximum);

    // ensure that the supported buffer size range obtained from CoreAudio makes
    // sense:
  ...
  using __coreaudio_callback_t = function<void(
      audio_device &, audio_device_io<__coreaudio_native_sample_type> &)>;
...
public:
  ...
  get_default_io_device(AudioObjectPropertySelector selector) {
    AudioObjectPropertyAddress pa = {selector, kAudioObjectPropertyScopeGlobal,
                                     kAudioObjectPropertyElementMain};
  ...
  template <typename Condition> auto get_device_list(Condition condition) {
  ...
  auto get_input_device_list() {
    return get_device_list([](const audio_device &d) { return d.is_input(); });
  }

  auto get_output_device_list() {
    return get_device_list([](const audio_device &d) { return d.is_output(); });
  }
...
private:
  ...
  static vector<AudioDeviceID> get_device_ids() {
    ...
    uint32_t data_size = 0;
    if (!__coreaudio_util::check_error(AudioObjectGetPropertyDataSize(
            kAudioObjectSystemObject, &pa, 0, nullptr, &data_size)))
      return {};

    const auto device_count =
        static_cast<uint32_t>(data_size / sizeof(AudioDeviceID));
    if (device_count == 0)
      return {};
    ...
    if (!__coreaudio_util::check_error(
            AudioObjectGetPropertyData(kAudioObjectSystemObject, &pa, 0,
                                       nullptr, &data_size, device_ids.data())))
      return {};
  ...
  static string get_device_name(AudioDeviceID device_id) {
    AudioObjectPropertyAddress pa = {kAudioDevicePropertyDeviceName,
                                     kAudioObjectPropertyScopeGlobal,
                                     kAudioObjectPropertyElementMain};
  ...
  static __coreaudio_stream_config
  get_device_io_stream_config(AudioDeviceID device_id) {
    return {
  ...
  get_device_stream_config(AudioDeviceID device_id,
...
struct __coreaudio_device_config_listener {
  static void register_callback(audio_device_list_event event,
                                function<void()> cb) {
...
private:
  ...
  __coreaudio_device_config_listener() {
    ...
    coreaudio_add_internal_callback<
        kAudioHardwarePropertyDefaultOutputDevice>();
  }
  ...
  void coreaudio_add_internal_callback() {
    ...
    if (!__coreaudio_util::check_error(AudioObjectAddPropertyListener(
            ...
            &coreaudio_internal_callback<selector>, this))) {
      assert(false); // failed to register device config listener!
    }
  }

  template <AudioObjectPropertySelector selector>
  static OSStatus coreaudio_internal_callback(
      AudioObjectID device_id, UInt32 /* inNumberAddresses */,
      const AudioObjectPropertyAddress * /* inAddresses */,
      void *void_ptr_to_this_listener) {
    __coreaudio_device_config_listener &this_listener =
        *reinterpret_cast<__coreaudio_device_config_listener *>(
            void_ptr_to_this_listener);
    this_listener.call<selector>();
    return {};
  }

  template <AudioObjectPropertySelector selector> void call() {
    if (auto cb_iter = callbacks.find(selector); cb_iter != callbacks.end()) {
      invoke(cb_iter->second);
    }
  }

  static constexpr AudioObjectPropertySelector
  get_coreaudio_selector(audio_device_list_event event) noexcept {
    switch (event) {
