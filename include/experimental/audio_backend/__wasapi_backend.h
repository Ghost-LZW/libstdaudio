...
public:
  ...
  static const IID &get_IAudioRenderClient_interface_id() {
    static const IID IAudioRenderClient_interface_id =
        __uuidof(IAudioRenderClient);
    return IAudioRenderClient_interface_id;
  }
  static const IID &get_IAudioCaptureClient_interface_id() {
    static const IID IAudioCaptureClient_interface_id =
  ...
  static string convert_string(const wchar_t *wide_string) {
    int required_characters = WideCharToMultiByte(CP_UTF8, 0, wide_string, -1,
    ...
    WideCharToMultiByte(CP_UTF8, 0, wide_string, -1, output.data(),
                        static_cast<int>(output.size()), nullptr, nullptr);
    return output;
  }

  static string convert_string(const wstring &input) {
    int required_characters = WideCharToMultiByte(
...
public:
  ...
  audio_device(audio_device &&other)
      : _device(other._device), _audio_client(other._audio_client),
        ...
        _running(other._running.load()), _name(std::move(other._name)),
  ...
  constexpr bool supports_sample_type() const noexcept {
    return is_same_v<_SampleType, float> || is_same_v<_SampleType, int32_t> ||
  ...
  template <typename _SampleType> bool set_sample_type() {
    if (_is_connected() && !is_sample_type<_SampleType>())
      throw audio_device_exception(
          "Cannot change sample type after connecting a callback.");
  ...
  template <typename _CallbackType,
            enable_if_t<is_nothrow_invocable_v<_CallbackType, audio_device &,
                                               audio_device_io<float> &>,
                        int> = 0>
  ...
  bool start(
    ...
    if (!_running) {
      ...
      REFERENCE_TIME buffer_duration =
          (ref_times_per_second * _buffer_frame_count) /
          _mix_format.Format.nSamplesPerSec;
      HRESULT hr = _audio_client->Initialize(
          AUDCLNT_SHAREMODE_SHARED,
          AUDCLNT_STREAMFLAGS_RATEADJUST | AUDCLNT_STREAMFLAGS_EVENTCALLBACK,
          buffer_duration, periodicity, &_mix_format.Format, nullptr);

      // TODO: Deal with AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED return code by
      // resetting the buffer_duration and retrying:
      // https://docs.microsoft.com/en-us/windows/desktop/api/audioclient/nf-audioclient-iaudioclient-initialize
      if (FAILED(hr))
        return false;

      /*HRESULT render_hr =*/_audio_client->GetService(
          __wasapi_util::get_IAudioRenderClient_interface_id(),
          reinterpret_cast<void **>(&_audio_render_client));
      /*HRESULT capture_hr =*/_audio_client->GetService(
          __wasapi_util::get_IAudioCaptureClient_interface_id(),
          reinterpret_cast<void **>(&_audio_capture_client));
  ...
  template <typename _CallbackType,
            enable_if_t<is_invocable_v<_CallbackType, audio_device &,
                                       audio_device_io<float> &>,
                        int> = 0>
  void process(const _CallbackType &callback) {
    if (!_mix_format_matches_type<float>())
      throw audio_device_exception(
          "Attempting to process a callback for a sample type that does not "
          "match the configured sample type.");
...
private:
  ...
  void _init_audio_client() {
    HRESULT hr = _device->Activate(
        __wasapi_util::get_IAudioClient_interface_id(), CLSCTX_ALL, nullptr,
  ...
  void _init_mix_format() {
    ...
    auto *device_mix_format_ex =
        reinterpret_cast<WAVEFORMATEXTENSIBLE *>(device_mix_format);
  ...
  void _fixup_mix_format() {
    _mix_format.Format.nBlockAlign =
        _mix_format.Format.nChannels * _mix_format.Format.wBitsPerSample / 8;
    _mix_format.Format.nAvgBytesPerSec = _mix_format.Format.nSamplesPerSec *
                                         _mix_format.Format.wBitsPerSample *
                                         _mix_format.Format.nChannels / 8;
  }
  ...
  void _process_helper(const _CallbackType &callback) {
    ...
    if (is_output()) {
      ...
      device_io.output_buffer = {
          reinterpret_cast<_SampleType *>(data), num_frames_available,
          _mix_format.Format.nChannels, contiguous_interleaved};
      ...
      DWORD flags = 0;
      BYTE *data = nullptr;
      _audio_capture_client->GetBuffer(&data, &next_packet_size, &flags,
                                       nullptr, nullptr);
      if (data == nullptr)
        return;

      audio_device_io<_SampleType> device_io;
      device_io.input_buffer = {reinterpret_cast<_SampleType *>(data),
                                next_packet_size, _mix_format.Format.nChannels,
  ...
  bool _is_connected() const noexcept {
    ...
    return visit([](auto &&callback) { return static_cast<bool>(callback); },
                 _user_callback);
  }
  ...
  using __wasapi_float_callback_t =
      function<void(audio_device &, audio_device_io<float> &)>;
  using __wasapi_int32_callback_t =
      function<void(audio_device &, audio_device_io<int32_t> &)>;
  using __wasapi_int16_callback_t =
      function<void(audio_device &, audio_device_io<int16_t> &)>;
  variant<__wasapi_float_callback_t, __wasapi_int32_callback_t,
          __wasapi_int16_callback_t>
      _user_callback;
...
private:
  ...
  static optional<audio_device> get_default_device(bool output_device) {
    ...
    HRESULT hr = CoCreateInstance(
        __wasapi_util::get_MMDeviceEnumerator_classid(), nullptr, CLSCTX_ALL,
    ...
    hr = enumerator->GetDefaultAudioEndpoint(output_device ? eRender : eCapture,
                                             eConsole, &device);
    if (FAILED(hr))
      return nullopt;
  ...
  static vector<IMMDevice *> get_devices(bool output_devices) {
    ...
    hr = enumerator->EnumAudioEndpoints(selected_data_flow, DEVICE_STATE_ACTIVE,
                                        &device_collection);
    if (FAILED(hr))
      return {};
...
public:
  ...
  void register_callback(audio_device_list_event event, F &&callback) {
    _callback_monitors[static_cast<int>(event)].reset(
        new WASAPINotificationClient{_enumerator, event, std::move(callback)});
  }
...
private:
  ...
  public:
    WASAPINotificationClient(IMMDeviceEnumerator *enumerator,
        ...
        : _enumerator(enumerator), _event(event),
          _callback(std::move(callback)) {
      if (_enumerator == nullptr)
        throw audio_device_exception(
            "Attempting to create a notification client for a null enumerator");
    ...
    HRESULT STDMETHODCALLTYPE OnDefaultDeviceChanged(
        EDataFlow flow, ERole role, [[maybe_unused]] LPCWSTR device_id) {
    ...
    OnDeviceAdded([[maybe_unused]] LPCWSTR device_id) {
    ...
    HRESULT STDMETHODCALLTYPE
    OnDeviceRemoved([[maybe_unused]] LPCWSTR device_id) {
      if (_event != audio_device_list_event::device_list_changed)
        return S_OK;
    ...
    HRESULT STDMETHODCALLTYPE OnDeviceStateChanged(
        [[maybe_unused]] LPCWSTR device_id, [[maybe_unused]] DWORD new_state) {
      if (_event != audio_device_list_event::device_list_changed)
        return S_OK;
    ...
    HRESULT STDMETHODCALLTYPE
    OnPropertyValueChanged([[maybe_unused]] LPCWSTR device_id,
                           [[maybe_unused]] const PROPERTYKEY key) {
      return S_OK;
    }

    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid,
                                             VOID **requested_interface) {
