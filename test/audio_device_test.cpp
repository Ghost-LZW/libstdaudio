...
TEST_CASE(
    "If there are any input devices, there must be a default input device") {
...
TEST_CASE(
    "If there are any output devices, there must be a default output device") {
  ...
  if (default_device.has_value()) {
    auto result =
        std::find_if(devices.begin(), devices.end(),
                     [&default_device](const audio_device &device) {
...
TEST_CASE("On any platform that supports multithreading, all input devices "
          "must support connect") {
...
TEST_CASE("On any platform that supports multithreading, all output devices "
          "must support connect") {
  if (std::thread::hardware_concurrency()) {
    auto devices = get_audio_output_device_list();
    for (auto &device : devices) {
...
TEST_CASE("Starting an input device and registering both a start and a stop "
          "callback") {
  auto devices = get_audio_input_device_list();
  for (auto &device : devices) {
    auto start_cb = [](audio_device &) noexcept {};
...
TEST_CASE("Starting an output device and registering both a start and a stop "
          "callback") {
  auto devices = get_audio_output_device_list();
  for (auto &device : devices) {
    auto start_cb = [](audio_device &) noexcept {};
